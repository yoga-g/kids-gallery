#!/usr/bin/env python3
"""
Quick add artwork to Yoga's gallery.
...
Images are saved in two versions:
  images/artworks/yoga/           ← watermarked (pushed to GitHub)
  images/artworks/yoga/_original/ ← clean originals (local only, gitignored)
"""
...
def main():
    parser = argparse.ArgumentParser(description="Add artwork to Yoga's gallery")
    ...
    print(f"\n✅ Added {len(new_entries)} artwork(s) for Yoga")
    ...
    if args.push:
        ...
        os.chdir(SCRIPT_DIR)
        os.system("git add .")
        os.system(f'git commit -m "Add {len(new_entries)} artwork(s) for Yoga"')
        os.system("git push")
